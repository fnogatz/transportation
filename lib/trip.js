...
Trip.prototype.getPositions = function getPositions () {
  ...
  return addResultMethods(points)

  function addResultMethods (obj) {
    obj.toGeoJSON = geo.toGeoJSON
    return obj
  }
}
...
function setDistance (points) {
  ...
  var distance
  var currPoint
  var prevPoint = turf.point(geo.toLonLat(points[0]))
  points[0].distance = points.travelDistance = 0

  for (var i = 1; i < points.length; i++) {
    currPoint = turf.point(geo.toLonLat(points[i]))
...
function setTimes (points, from, to) {
  ...
  var distanceAll = turf.lineDistance(turf.linestring(points.slice(from, to + 1).map(geo.toLonLat)), 'kilometers')

  var distance
  for (var i = from + 1; i < to; i++) {
    distance = turf.lineDistance(turf.linestring(points.slice(from, i + 1).map(geo.toLonLat)), 'kilometers')
